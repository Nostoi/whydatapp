...
- **Upgrades must not require manual fixup.** Schema migrations run automatically with backups; `~/.why/config.toml` keys are deep-merged over current defaults so new keys appear without user action; new shell-hook content lands when `why init` is re-run, but old hooks should keep working until then. Any new schema column must be backwards-compatible (`DEFAULT 0` / `DEFAULT NULL`, no NOT NULL without a default, no constraint changes) so that a user upgrading from an older version gets a seamless `migrate()` call with no data loss and no manual SQL to run.
